...
export const Dashboard: React.FC<DashboardProps> = ({ data, onBack }) => {
    ...
    const renderPieChart = (counts: Record<string, number>, onSelect?: (key: string) => void) => {
        ...
        if (total === 0) return <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>データなし</div>;
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
                <div style={{ position: 'relative', width: '200px', height: '200px' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>合計</div>
                        <div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--text-main)' }}>{total}</div>
                    </div>
                </div>
                ...
                <div style={{ width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    {slices.map((s) => (
                        <div key={s.key}
                            ...
                            style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem', cursor: onSelect ? 'pointer' : 'default', padding: '4px', borderRadius: '4px', transition: 'background 0.2s', color: 'var(--text-main)' }}>
    ...
    const renderBarList = (counts: Record<string, number>, color: string, onSelect?: (key: string) => void) => {
        ...
        if (sorted.length === 0) return <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>データなし</div>;
        ...
        return sorted.slice(0, 10).map(([key, val]) => (
            <div key={key} style={{ marginBottom: '12px', cursor: onSelect ? 'pointer' : 'default' }} onClick={() => onSelect?.(key)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginBottom: '4px' }}>
                    ...
                    <span style={{ color: 'var(--text-muted)' }}>{val}件</span>
                </div>
                <div style={{ width: '100%', height: '8px', background: 'var(--bg-overlap)', borderRadius: '4px', overflow: 'hidden' }}>
                    <div style={{
    ...
    return (
        <div className="view active dash-view" style={{ overflowY: 'auto', flex: 1 }}>
            <div className="dashboard-container" style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        <button onClick={onBack} className="secondary-btn" style={{ gap: '8px', background: 'var(--card-bg)', border: '1px solid var(--card-border)', color: 'var(--text-main)' }}>
                            <ArrowLeft size={16} /> 戻る
                        </button>
                        <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--text-main)' }}>集計ダッシュボード</h2>
                    </div>

                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        <div style={{ display: 'flex', background: 'var(--input-bg)', padding: '4px', borderRadius: '8px', border: '1px solid var(--input-border)' }}>
                            {(['all', 'year', 'month', 'week', 'day'] as TimeRange[]).map((r) => (
                                <button key={r} onClick={() => setTimeRange(r)}
                                    style={{
                                        ...
                                        color: timeRange === r ? 'var(--primary)' : 'var(--text-muted)',
                        ...
                        <div style={{ display: 'flex', gap: '5px' }}>
                            <button onClick={() => setChartType('bar')} className="secondary-btn" style={{
                                padding: '8px', background: chartType === 'bar' ? 'var(--primary)' : 'var(--card-bg)', color: chartType === 'bar' ? 'white' : 'var(--text-muted)', border: '1px solid var(--card-border)'
                            }}>
                                <BarChart3 size={18} />
                            </button>
                            <button onClick={() => setChartType('pie')} className="secondary-btn" style={{
                                padding: '8px', background: chartType === 'pie' ? 'var(--primary)' : 'var(--card-bg)', color: chartType === 'pie' ? 'white' : 'var(--text-muted)', border: '1px solid var(--card-border)'
                            }}>
                ...
                {activeFilters.length > 0 && (
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center', padding: '10px', background: 'var(--bg-overlap)', borderRadius: '8px', border: '1px dashed var(--card-border)' }}>
                        <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 'bold' }}>絞り込み中:</span>
                        {activeFilters.map(f => (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: '20px' }}>
                    <div className="dash-panel" style={{ background: 'var(--card-bg)', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px -1px var(--card-shadow)', border: '1px solid var(--card-border)' }}>
                        <h3 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', color: 'var(--text-main)', borderBottom: '2px solid var(--bg-overlap)', paddingBottom: '10px' }}>
                    ...
                    <div className="dash-panel" style={{ background: 'var(--card-bg)', padding: '24px', borderRadius: '12px', boxShadow: '0 4px 6px -1px var(--card-shadow)', border: '1px solid var(--card-border)', gridColumn: '1 / -1' }}>
                        <h3 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', color: 'var(--text-main)', borderBottom: '2px solid var(--bg-overlap)', paddingBottom: '10px' }}>
